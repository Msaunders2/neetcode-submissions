class Solution {
    ...
    exist(board, word) {
        ...
        function backtrack(row, col, wordInd){
            ...
            //move up row
            if(row + 1 < board.length) backtrack(row + 1,col, wordInd + 1);

            //move down row
            if(row - 1 >= 0) backtrack(row - 1,col, wordInd + 1);

            //move up on col
            if(col + 1 < board[0].length) backtrack(row,col + 1, wordInd + 1);
            
            //move down col
            if(col - 1 >= 0) backtrack(row,col - 1, wordInd + 1);
